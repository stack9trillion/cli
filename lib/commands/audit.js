...
class VerifySignatures {
  ...
  async verifySignatures (name, version, registry) {
    ...
    if (dist.attestations) {
      // TODO does this url match our registry?
      const url = new URL(dist.attestations.url)
      const keys = this.keys.get(registry)
      const { attestations } = await fetch.json(url.pathname, {
        ...this.npm.flatOptions, registry,
      })
      for (const { bundle } of attestations) {
        // TODO needs to catch and add to this.invalid with a code that the
        // output code can filter on to print the appropriate message
        await sigstore.verify(bundle, null, {
          getPublicKey: (keyid) => {
            const key = keys.find(k => k.keyid === keyid)
            return key && key.pemkey
          },
        })
      }
    }
