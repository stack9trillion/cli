...
class CertificateChainVerifier {
    ...
    verify() {
        if (this.certs.length === 0) {
            throw new error_1.CertificateChainVerificationError('No certificates provided');
        }
    ...
    sort() {
        ...
        if (paths.length === 0) {
            throw new error_1.CertificateChainVerificationError('No trusted certificate path found');
        }
    ...
    buildPaths(certificate) {
        ...
        if (issuers.length === 0) {
            throw new error_1.CertificateChainVerificationError('No valid certificate path found');
        }
    ...
    checkPath(path) {
        if (path.length < 2) {
            throw new error_1.CertificateChainVerificationError('Certificate chain must contain at least two certificates');
        }
        // Check that all certificates are valid at the check date
        const validForDate = path.every((cert) => cert.validForDate(this.validAt));
        if (!validForDate) {
            throw new error_1.CertificateChainVerificationError('Certificate is not valid or expired at the specified date');
        }
        // Ensure that all certificates beyond the leaf are CAs
        const validCAs = path.slice(1).every((cert) => cert.isCA);
        if (!validCAs) {
            throw new error_1.CertificateChainVerificationError('Intermediate certificate is not a CA');
        }
        // Certificate's issuer must match the subject of the next certificate
        // in the chain
        for (let i = path.length - 2; i >= 0; i--) {
            if (!path[i].issuer.equals(path[i + 1].subject)) {
                throw new error_1.CertificateChainVerificationError('Incorrect certificate name chaining');
            }
        }
    }
}
